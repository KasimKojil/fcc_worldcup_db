...
echo "$($PSQL "SELECT SUM(winner_goals + opponent_goals) AS goals_combined from games;")"
...
echo "$($PSQL "SELECT MAX(GREATEST(winner_goals, opponent_goals)) FROM games")"
      #menggunakan fungsi greatest untuk mengembalikan nilai tertinggi dari masing masing kolom
      #kemudian memakai fungsi max ke greatest, untuk dapat nilai paling tinggi
...
echo "$($PSQL "SELECT games.year, teams.name FROM games RIGHT JOIN teams ON teams.team_id = games.winner_id where round = 'Final' ORDER BY year")"
